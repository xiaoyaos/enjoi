...
const enjoi = function (schema, options = {}) {
    ...
    function resolvetype(current) {
        ...
        function joitype(type, format) {
            ...
            switch (type) {
                case 'lazy':
                    joischema = lazy(current);
                    break;
                case 'array':
                    joischema = array(current);
                    break;
                case 'boolean':
                    joischema = boolean(current);
                    break;
                case 'integer':
                case 'number':
    ...
    function lazy(current){
      let joischema = Joi.lazy(eval(current.fun));
      return joischema;
    }
    ...
    function object(current) {
        ...
        current.nullable && (joischema = joischema.allow(null));
        if(current.pattern){
          joischema = pattern(current, joischema);
        }

        if (current.when) {
          for(let p of current.when){
            let then;
            if("required" === p.then){
              then = Joi.required();
            }
            joischema = joischema.when(p.key,{is:p.is,then:then});
          }
        }
    ...
    function boolean(current){
      let joischema = Joi.boolean();
      current.nullable && (joischema = joischema.allow(null));
      if(current.truthy){
        joischema = joischema.truthy(current.truthy);
      }
      if(current.falsy){
        joischema = joischema.falsy(current.falsy);
      }
      return joischema;
    }
    ...
    function array(current) {
        ...
        if (current.items) {
            for(let i = 0; i <= current.items.length-1; i++){
              items.push(resolveAsArray(current.items[i]));
            }
    ...
    function string(current) {
        ...
        if (current.enum) {
            // return Joi.any().valid(current.enum);
            joischema = joischema.valid(current.enum);
        }

        if (current.replace) {
          for(let p of current.replace){
            joischema = joischema.replace(p.pattern,p.replacement);
          }
        }
    ...
    function pattern(current, joischema){
      let regex = new RegExp(current.pattern.regex);
      let joitype = current.pattern.swagger.type == 'any'?eval('Joi.'+current.pattern.swagger.type+'()') : resolve(current.pattern.swagger);
      joischema = joischema.pattern(regex,joitype);
      current.nullable && (joischema = joischema.allow(null));
      return joischema;
    }
